...
The `SWIRL_EXPLAIN` item determines if Swirl will show the [explain structure](5.-Developer-Guide#understand-the-explain-structure) or not.
...
:warning: Please [contact support](#support) for instructions on upgrading Docker containers!
...
| SWIRL_Q_WAIT | The number of seconds to wait before redirecting to the Result Mixer after using the [q= parameter](5.-Developer-Guide#create-a-search-object-with-the-q-url-parameter) | `SWIRL_Q_WAIT = 7` |
| SWIRL_RERUN_WAIT | The number of seconds to wait before redirecting to the Result Mixer when [re-running a search](5.-Developer-Guide#re-run-a-search) | `SWIRL_RERUN_WAIT = 8` |
| SWIRL_SUBSCRIBE_WAIT | The number of seconds to wait before timing out and reporting an error when [updating a search](5.-Developer-Guide#update-a-search) | `SWIRL_SUBSCRIBE_WAIT = 20` |
| SWIRL_DEDUPE_FIELD | The field to use when [detecting and removing duplicates](5.-Developer-Guide#detect-and-remove-duplicate-results) with the `DedupeByFieldPostResultProcessor` | `SWIRL_DEDUPE_FIELD = 'url'` |
| SWIRL_DEDUPE_SIMILARITY_MINIMUM | The minimum similarity score that constitutes a duplicate, when [detecting and removing duplicates](5.-Developer-Guide#detect-and-remove-duplicate-results) with the `DedupeBySimilarityPostResultProcessor` | `SWIRL_DEDUPE_SIMILARITY_MINIMUM = 0.95` |
| SWIRL_DEDUPE_SIMILARITY_FIELDS | A list of the fields to use when determining the similarity between documents when [detecting and removing duplicates](5.-Developer-Guide#detect-and-remove-duplicate-results) with the `DedupeBySimilarityPostResultProcessor` | `SWIRL_DEDUPE_SIMILARITY_FIELDS = ['title', 'body']` | 
...
Temporary changes can also be made via the Django Console here: http://localhost:8000/admin/django_celery_beat/crontabschedule/ 
...
When one or more Search objects [have the subscribe property set to True](5.-Developer-Guide#subscribe-to-a-search), Swirl will periodically [update that Search](5.-Developer-Guide#update-a-search).
...
```
__S_W_I_R_L__2_._5______________________________________________________________

Service: redis...RUNNING, pid:26365
Service: django...RUNNING, pid:26391
Service: celery-worker...RUNNING, pid:26424

  PID TTY           TIME CMD
26365 ttys001    0:26.45 redis-server *:6379 
26391 ttys001    0:14.22 /Users/erikspears/.pyenv/versions/3.11.4/bin/python3.11 /Users/erikspears/.pyenv/versions/3.11.4/bin/daphne -b 0.0.0.0 -p 8000 swirl_server.asgi:application
26424 ttys001    0:39.76 /Users/erikspears/.pyenv/versions/3.11.4/bin/python3.11 /Users/erikspears/.pyenv/versions/3.11.4/bin/celery -A swirl_server worker --loglevel INFO
...
* To change startup incantations, services names, or default value for any service, edit the `SWIRL_SERVICES` variable in `swirl/services.py` (*the rabbitmq settings are preserved only for backward compatibility*):
...
:star: When Django is running [the front page should load...](http://localhost:8000/swirl/)
...
This is only required if using the [Search Expiration Service](#search-expiration-service).
...
:star: The URL is: http://localhost:8000/admin/ 
...
:star: To configure PostgreSQL as the Django back-end:
...
The `settings.py` file also contains the configuration for the [Search Expiration Service](#search-expiration-service).
...
| logs/celery-beats.log | Contains the log of the celery-beats service, which is only used by the [Search Expiration Service](#search-expiration-service) | Look here for issues with expiration not working | 
...
:key: For more information about logging, see: [https://docs.python.org/3/howto/logging.html](https://docs.python.org/3/howto/logging.html)
