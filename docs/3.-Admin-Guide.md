...
{: .warning }
As of Release 2.6, Swirl's start-up process no longer starts `redis`. You must now have `redis` installed and runnning before starting Swirl.
...
{: .highlight }
The FDQN that Swirl should listen on must be the first entry in the `ALLOWED_HOST` list.
