...
{: .warning }
As of verion 2.5, the revised `CosineRelevancyResultProcessor` must be added *last* to the list of `result_processors`.
...
{: .highlight }
For Release 2.5.1, [`requests.py`](https://github.com/swirlai/swirl-search/blob/main/swirl/connectors/requests.py) was udpated to handle XML responses from source APIs and convert them to JSON for mapping in SearchProvider configurations.
