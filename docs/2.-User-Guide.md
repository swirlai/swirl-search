...
This will limit the query to SearchProviders with tag `company`. For more information, see the section on [Organizing SearchProviders with Active, Default and Tags](#organizing-searchproviders-with-active-default-and-tags).
...
For more details please consult the Developer Guide [Configure Relevancy Field Weights](5.-Developer-Guide#configure-relevancy-field-weights) and [Understand the Explain Structure](5.-Developer-Guide#understand-the-explain-structure) sections.
...
```
    "body": "<em>Performance</em> <em>management</em> is the process of setting goals and expectations for employees and then tracking and measuring their progress. This can be done through regular one-on-one meetings, <em>performance</em> reviews, and other feedback mechanisms. The goal of <em>performance</em> <em>management</em> is to help employees improve their work and contribute to the overall success of the organization.",
```
As of version 1.10, Swirl can also integrate source synonym configurations into relevancy calculations with corresponding hit highlighting.  See the Developer Guide [Integrate Source Synonyms Into Swirl Relevancy](5.-Developer-Guide#integrate-source-synonyms-into-swirl-relevancy)
section for details.
...
| elastic_cloud.json | elasticsearch, cloud version | [Enron Email Dataset](6.-Developer-Reference#enron-email-data-set) requires cloud_id, credentials |
| elasticsearch.json | elasticsearch, local install | [Enron Email Dataset](6.-Developer-Reference#enron-email-data-set) requires host, port, credentials | 
...
| funding_db_bigquery.json | BigQuery funding database  | [Funding Dataset](6.-Developer-Reference#funding-data-set) |
| funding_db_postgres.json  | PostgreSQL funding database | [Funding Dataset](6.-Developer-Reference#funding-data-set) |
| funding_db_sqlite3.json  | SQLite3 funding database  | [Funding Dataset](6.-Developer-Reference#funding-data-set) |
...
| microsoft.json | Searches M365 Outlook Messages, Calendar Events, OneDrive Files, SharePoint Sites, and Teams Chat | See the [M365 Guide](4.-M365-Guide) for details |
...
| opensearch.json  | OpenSearch 2.x | [Developer Guide](6.-Developer-Reference#elastic--opensearch) |
...
:start: As of Release 2.6, Swirl includes SearchProviders for ServiceNow (Knowledge and Service Catalog), Google News and a searchable version of the Hacker News feeds.
...
| Tags | List of strings that organize providers into groups. Tags can be specified in combination with SearchProvider names and/or id's in `Search.searchprovider_list`, in the `providers=` [URL parameter](5.-Developer-Guide#create-a-search-object-with-the-q-url-parameter), or in a query in the form `tag:term` |
...
The `query_string` is provided by Swirl as described in the [Developer Guide](5.-Developer-Guide#workflow)
...
Consult the Developer Guide for details on how to [Develop New Connector](5.-Developer-Guide#develop-new-connectors).
...
[Result mixers](6.-Developer-Reference#mixers-1) further manipulate and re-organize the data from multiple results.
...
* Read the [Admin Guide](3.-Admin-Guide), the [M365 Guide](4.-M365-Guide), the [Developer Guide](5.-Developer-Guide), and the [Develper Reference](6.-Developer-Reference).
