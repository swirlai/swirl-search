...
This guide is intended for use by M365 Administrators with authority to add a new App Registration in their Azure Portal and optionally grant permissions for various APIs as noted below on behalf of users who wish to use Swirl Metasearch to query their personal M365 content.
...
After the new App is configured, you can use it to authenticate users with OIDC and allow them to search their personal MS365 content using OAUTH2 permissions consent.

<br/>

## Before you begin
You need the following information from Swirl deployment:
...
**In addition, in order to use OIDC and OAUTH2 your deployment must be using https, the only exception being where localhost is used as the host, one can use `http://` It is important to keep in mind that the URLs for Single Page Applications and Web Protocols do not permit Fully Qualified Domains without `https://`**
...
**NOTE**: After the secret has been created it is important that you copy the value shown on the screen at that time as it will be obscured from this point on when returning to this menu.
...
Edit the Swirl `.env` file (or `.env.docker` if running Docker) as shown in the example below. Note: The `.env` file is created after running `install.sh`, for local installations, but is always present in Docker.
...
* Find each Microsoft 365 SearchProvider that you wish to activate. There are separate providers for Outlook Messages, OneDrive files, etc. Edit the SearchProvider by adding the `id` of the provider you wish to edit to the URL. For example, to edit the provider with an id of `11`, go to: <http://localhost:8000/searchproviders/11>

![Swirl SearchProvider](images/swirl_sp_m365.png)

* Set the form at the bottom of the page to "RAW" mode. Then edit the form, changing the `active` property from `false` to `true`:
...
Click on the profile icon at the top right. Depending on how the above configuration was done, M365 may require authentication and/or granting of access. After that, the toggle next to the Microsoft logo will light up indicating successful connection.

Now, enter a search like:
