...
This guide is intended to provide developers with an overview of Swirl and how to accomplish specific tasks with it. Please refer to the [Developer Reference](6.-Developer-Reference) for lists of system states, system objects and properties, etc.
...
Important: Note the `id` value down. It's required in the next step to get unified, relevancy ranked results.

## Create a Search Object with the "q=" URL Parameter

To create a Search object and specify only a `query_string`, accepting the defaults otherwise, add `?q=your-query-string` to the API URL.
...
:key: There are some limitations to the `q=` interface:
...
* All active SearchProviders are queried
...
The `qs=` parameter works like the `q=` parameter (*see above*) does, except that it returns the first page of results (only) to the caller directly, with no polling of the Search object or handling of a redirect required.
...
The `qs=` parameter can also be used with the [providers](#specify-searchproviders-with-the-providers-url-parameter) and [result_mixer](6.-Developer-Reference/#mixers-1) parameters.
...
One way to address this is to make sure the [NOT query-mapping](2.-User-Guide#query-mappings) is set correctly for that provider.
...
Swirl will set the `status` to "FULL_UPDATE_READY" when finished updating. New results will have a field `new` which will be set to 1. Use the `new_result_url` to retrieve new results via the `ResultNewItemsMixer` or `DateNewItemsMixer`, depending on the `sort` specified.
...
Use the [NewItems Mixers](6.-Developer-Reference/#mixers-1) to view only new results for a Search. 

## Subscribe to a Search with M365 Sources

:warning: To subscribe to a Search that contains any Microsoft SearchProviders, follow these steps _BEFORE_ setting the `subscribe` field to `true` on the Search object.
...
:key: If you don't see the Microsoft option on this page, make sure that at least one M365 SearchProvider enabled (`"active": true`) before proceeding to the next step.
...
Use the [`RelevancyNewItemsMixer` and `DateNewItemsMixer`](6.-Developer-Reference/#mixers-1) to retrieve new, updated results.
...
:warning: Use Spellcheck cautiously as it tends to cause a lack of results from sources that have sparse indexes and limited or no fuzzy search.

## Expire Search Objects

If your Swirl installation is using the [Search Expiration Service](3.-Admin-Guide#search-expiration-service), users can specify the retention setting for each Search. 
...
The exact time of expiration is determined by the [Celery Beat Configuration](3.-Admin-Guide/#configuring-celery--redis) and the [Search Expiration Service](3.-Admin-Guide#search-expiration-service) configuration.
...
If you need more mixed results, increase the `results_per_query` value in the SearchProviders. A setting of 20 or 50 or 100 
will ensure that you have lots of results to page through with the mixer.

Since Swirl will only page through saved Results, re-run the query with a higher `results_per_query` value in each SearchProvider to get more results.
...
The [CosineRelevancyProcessor](6.-Developer-Reference/#cosinerelevancypostresultprocessor) outputs a JSON structure that explains the `swirl_score` for each result. It is displayed by default; to hide it add `&explain=False` to any mixer URL.
...
:warning: To connect to a new endpoint for an existing Connector - like RequestsGet - create a new SearchProvider instead. The [Google PSE SearchProvider example JSON](https://github.com/swirlai/swirl-search/blob/main/SearchProviders/google_pse.json) shows how to use one connector to make hundreds of SearchProviders!
...
* Add new processors to the appropriate CHOICES section of [swirl/models.py](https://github.com/swirlai/swirl-search/tree/main/swirl/models.py) - note this will require [database migration](3.-Admin-Guide#database-migration) 
...
* Results from each source should be processed with a result processor, ideally the [MappingResultProcessor](6.-Developer-Reference/#result-processors).
...
* Add new processors to the appropriate `CHOICES` part of [swirl/models.py](https://github.com/swirlai/swirl-search/tree/main/swirl/models.py) - note this will require [database migration](3.-Admin-Guide#database-migration) 
...
* Add new mixers to the appropriate `CHOICES` section of [`swirl/models.py`](https://github.com/swirlai/swirl-search/tree/main/swirl/models.py) - note this will require [database migration](3.-Admin-Guide#database-migration) 
...
:star: New in Swirl 1.10:  Use hit highlight extraction to integrate a SearchProvider's synonym hits into Swirl's relevancy processing.
...
1. Update the default SearchProvider configuration to add hit highlighting to the source fields that are mapped to Swirl's `title` and `body` fields.
...
2. In the `results_mapping` field of the SearchProvider, asssign the Swirl fields `body_hit_highlights` and `title_hit_highlights` to the JSON reference in which the source's hit highlighting resides.
...
```
Swirl may rewrite these queries depending on the SearchProvider `query_mappings`. See [Search Syntax](2.-User-Guide/#search-syntax) for more information.
...
The retention setting will cause the search to be deleted after 1 hour, assuming the [Search Expiration Service](3.-Admin-Guide#search-expiration-service) is running.

Here are examples that will work if the [Funding Dataset](6.-Developer-Reference/#funding-data-set) is installed:
