...
class RAGPostResultProcessor(PostResultProcessor):
    ...
    def background_process(self):
        ...
        # DS-5598: temporary diagnostic — qa-suite RAG scenarios render
        # the "Generated in Xs" fallback in Galaxy's AI Summary footer
        # instead of "Generated by <model> in Xs". The Galaxy bundle
        # already carries PR #261's un-clobber fix, and this writer sets
        # `additional_content.ai_model` to a non-empty string, so the
        # value must be getting dropped somewhere downstream
        # (serializer / response-on-the-wire / Galaxy parse). Log it at
        # WARNING level so it shows up in the test-build-pipeline qa-suite
        # log without enabling SWIRL_LOG_DEBUG. Remove after the
        # downstream issue is identified.
        logger.warning(
            f'RAG-DIAG client_model={client_model!r} '
            f'has_get_provider={hasattr(self.client, "get_provider")} '
            f'additional_content={rag_result["additional_content"]!r}'
        )
