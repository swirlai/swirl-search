...
class GenAIQueryProcessor(QueryProcessor):
    ...
    def set_guide_from_tags(self):
        ...
        if not self.system_guide:
            self.warning(
                f"No prompt provided, using default {MODEL_DEFAULT_SYSTEM_GUIDE}"
            )
    ...
    def set_do_filter_from_tags(self):
        ...
        if filter_tag_value is None or len(filter_tag_value) <= 0:
            ...
            try:
                if filter_tag_value.lower() == 'true':
                    ...
                    logger.error(
                        "Error parsing filter tag %s using default: %s",
                        filter_tag_value,
                        MODEL_DEFAULT_DO_FILTER,
                    )
                    self.do_filter = MODEL_DEFAULT_DO_FILTER
            except Exception as x:
                logger.error(
                    "Exception parsing filter tag %s using default: %s",
                    filter_tag_value,
                    MODEL_DEFAULT_DO_FILTER,
                )
                self.do_filter = MODEL_DEFAULT_DO_FILTER
    ...
    def process(self, client=None):
        try:
            ...
            if not self.do_filter:
                ...
                logger.info("applying legacy filters to response, it may not be used")
            ...
            if len(message) > 1.25 * len(self.query_string):
                self.warning(
                    f"{self}: ChatGPT response more than 5x query string length, ignoring: {message}"
                )
                return self.query_string
            ...
            if '1.' in message:
                # '\n\n1. cryptocurrency\n2. Bitcoin\n3. blockchain\n4. Ethereum\n5. Litecoin'
                term_list = [
                    term.strip().split('.')[1].strip()
                    for term in message.strip().split('\n')
                    if term.strip()
                ]
            ...
            logger.error(
                "unexpected exception while rewriting query, returning original"
            )
            return self.query_string
