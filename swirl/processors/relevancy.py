...
class CosineRelevancyResultProcessor(ResultProcessor):
    ...
    def __init__(self, results, provider, query_string, request_id=''):
        ...
        return super().__init__(results,provider,query_string,request_id)
    ...
    def process(self):
        ...
        dict_result_lens = {}
        list_query_lens = []
        swrel_logger = SwirlRelevancyLogger(self.request_id, self.results.searchprovider +'_'+ self.results.provider_id)
        swrel_logger.start_pass_1()
        highlighted_json_results = []

        if not (self.results and self.results.json_results):
            return
        parsed_query = parse_query(self.result.query_string, self.result.result_processor_json_feedback)
            ...
            for field in RELEVANCY_CONFIG:
                if field in item:
                    ...
                    if match_any(parsed_query.query_stemmed_list, result_field_stemmed_list):
                        ...
                        query_nlp = nlp(query)
                        swrel_logger.end_nlp(len(query))
