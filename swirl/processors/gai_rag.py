...
class RAGPostResultProcessor(PostResultProcessor):
    ...
    def background_process(self):
        ...
        # LiteLLM maps all exceptions to OpenAIError
        # to do: review this
        except Exception as err:
            if DO_MESSAGE_MOCK_ON_ERROR:
                logger.error(f"Error creating GAI response: {err}")
    ...
    def process(self, should_return=True):
        ...
        if getattr(settings, 'OPENAI_API_KEY', None):
        ...
        elif getattr(settings, 'VERTEX_PROJECT', None):
        ...
        elif getattr(settings, 'AZURE_API_KEY', None):
            ...
            logger.error("RAG NO RAG API key set!!")
            # to do: maybe should warn and continue trying, what if the gai is local and no key/pw required?
            return 0
        if should_return:
