    ...
    def list(self, request):
        ...
        if query_string:
            ...
            try:
                ...
                new_search = Search.objects.create(query_string=query_string,searchprovider_list=providers,owner=self.request.user,pre_query_processors=[pre_query_processor_in])
            except Error as err:
                self.error(f'Search.create() failed: {err}')
            new_search.status = 'NEW_SEARCH'
            new_search.save()
